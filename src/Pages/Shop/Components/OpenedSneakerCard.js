...
const OpenedSneakerCard = () => {
    ...
    <div className='relative'>
        <Link to='/cart' className={cartItems.length > 0 ? `${styles.filter} hover:underline w-fit flex ml-auto sticky top-2` : 'invisible'}>CART ({cartItems.length})</Link>
        <LeftMenu />
        <Header />
        <div className={styles.openedcard}>
            ...
            <div className='w-[350px] flex flex-col mt-[70px]'>
                ...
                {cartItems.filter(item => item.id === id).length > 0 ? <button className={styles.addedbutton}>Added</button> : <button onClick={() => addItem({id, title, img, color, price})} className={styles.addbutton}>Add to cart</button>
}   
            </div>
        </div>
    </div>
